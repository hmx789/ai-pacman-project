    ...
    for x in range(DIGIT_DATUM_HEIGHT):
        ...
        for y in range(DIGIT_DATUM_WIDTH):
            ...
            if not d:
                ...
                d = 2
            if feature and pix and w:
                c2 = 1
            else:
                c2 = 0

            if pix and oFeat and cond1:
                f2 = 1
            else:
                f2 = 0
            if not l:
...
def enhancedPacmanFeatures(state, action):
    ...
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    successor = state.generateSuccessor(0, action)  # getting the successors given the action
    food_list = successor.getFood().asList()        # getting the list of foods from the successor
    for food in food_list:
        #using the manhattann distance of the food and position,update food in features
        s = successor.getPacmanPosition()
        distance = util.manhattanDistance(food,s)   #distance from the position and food
        food = str(food)        # used to access features
        features[food] = 1 / distance
